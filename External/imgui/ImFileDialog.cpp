...
#undef GUI_ELEMENT_SIZE
#define GUI_ELEMENT_SIZE 24.0f
...
namespace ifd {
	...
	bool PathBox(const char* label, std::filesystem::path& path, char* pathBuffer, ImVec2 size_arg) {
		...
		if (!(*state & 0b001)) {
			...
			for (auto comp : path) {
				...
				totalWidth += ImGui::CalcTextSize(section.c_str()).x + style.FramePadding.x * 2.0f + 24.0f;
	...
	void FileDialog::m_renderFileDialog()
	{
		...
		if (ImGui::IsWindowFocused(ImGuiFocusedFlags_RootAndChildWindows) && escapeKey >= 0 && ImGui::IsKeyPressed(ImGuiKey_Escape))
		{
			m_isOpen = false;
		}
