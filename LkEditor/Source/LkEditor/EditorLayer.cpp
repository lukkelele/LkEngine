...
namespace LkEngine {
	...
	void LEditorLayer::Initialize()
	{
		...
		EditorContext.Input.bMouseEnabled = UI::IsMouseEnabled();
		EditorContext.Input.bKeyboardEnabled = UI::IsKeyboardEnabled();
		...
		switch (SceneState)
		{
			case ESceneState::Edit:
			{
				...
				if (bRenderSkybox)
				{
					LRenderer::Submit([&]()
					{
						LOpenGL_Debug::RenderSkybox(EditorCamera.GetViewMatrix(), EditorCamera.GetProjectionMatrix());
					});
				}
	...
	void LEditorLayer::RenderUI()
	{
		...
		UI::Begin(PanelID::Sidebar1, nullptr, UI::SidebarFlags);
		{
			...
			if (ImGui::TreeNode("Viewport Debug Info"))
			{
	...
	void LEditorLayer::UI_ViewportTexture()
	{
		...
		TObjectPtr<LImage2D> ViewportImage = LRenderer::GetViewportFramebuffer()->GetImage(0);
	...
	void LEditorLayer::OpenProject(const std::filesystem::path& InProjectPath)
	{
		...
		{
			LK_CORE_ASSERT(false, "Project does not exist: {}", InProjectPath.string());
			return;
		}
		...
		FProjectSerializer ProjectSerializer(Project);
		...
		if (!ProjectHasScene)
		{
			LK_CORE_CONSOLE_INFO("Project does not have a starter scene, creating one");
			NewScene();
		}
		...
		if (EditorScene)
		{
			...
			if (LEntity CameraEntity = EditorScene->FindEntity("EditorCamera"))
			{
				...
				LK_CORE_VERIFY(CameraEntity.HasComponent<LCameraComponent>());
				...
				EditorCamera.FocalPoint = CameraRef.FocalPoint;
				EditorCamera.Pitch = CameraRef.Pitch;
				EditorCamera.Yaw = CameraRef.Yaw;
				LK_CORE_DEBUG_TAG("Editor", "Loaded editor camera data from the scene '{}'", EditorScene->GetName());
			}
			else
			{
				...
				CameraComp.ProjectionType = EditorCamera.GetProjectionType();
			}
		}
	...
	void LEditorLayer::SaveProject()
	{
		if (TObjectPtr<LProject> CurrentProject = LProject::Current(); CurrentProject != nullptr)
		{
			...
			ProjectSerializer.Serialize(Config.ProjectDirectory + "/" + Config.ProjectFileName);
			PanelManager->Serialize();
		}
		else
		{
	...
	void LEditorLayer::OnMouseEnabled(const bool Enabled)
	{
		...
		if (Enabled)
		{
			UI_SetWindowFocus(EditorContext.Input.LastToggleFocusedWindowID);
			if (LInput::GetCursorMode() != ECursorMode::Normal)
			{
				LInput::SetCursorMode(ECursorMode::Normal);
			}
		...
		else
		{
			...
			{
				EditorContext.Input.LastToggleFocusedWindowID = GImGui->HoveredWindow->ID;
				EditorContext.Input.LastToggleFocusedWindowName = GImGui->HoveredWindow->Name;
			}
		}

		EditorContext.Input.bMouseEnabled = Enabled;
	}
	...
	void LEditorLayer::SaveScene()
	{
		if (!SceneFilePath.empty())
		{
			...
			if (LEntity EditorCameraEntity = EditorScene->FindEntity("EditorCamera"); EditorCameraEntity.operator bool())
			{
				LTransformComponent& TransformComp = EditorCameraEntity.Transform();
				TransformComp.Translation = EditorCamera.Position;
				LK_CORE_WARN("Setting transform component's translation to editor camera position");
			}
	...
	void LEditorLayer::UI_ShowViewportAndWindowDetails()
	{
		...
		ImGui::BeginGroup();
		{
			...
			{
				const glm::vec2 CameraPos = EditorCamera.Position;
	...
	bool LEditorLayer::OpenScene(const std::filesystem::path& Filepath, const bool CheckForAutosaves)
	{
		...
		{
			LK_CORE_WARN_TAG("Editor", "Failed to deserialize scene: '{}'", Filepath);
		}
	...
	void LEditorLayer::SetScene(TObjectPtr<LScene> InScene)
	{
		...
		if (InScene && InScene->bEditorScene)
		{
			...
			//auto CameraView = EditorScene->GetAllEntitiesWith<LCameraComponent>();
			auto CameraView = InScene->GetAllEntitiesWith<LCameraComponent>();
			if (CameraView.empty())
			{
				LEntity CameraEntity = InScene->FindEntity("EditorCamera");
				if (CameraEntity.operator bool())
				{
					LK_CORE_ERROR("CAMERA ENTITY: {}", CameraEntity.GetUUID());
				}
				else
				{
				#if 0
					LK_CORE_WARN_TAG("Editor", "Scene '{}' is missing any camera components", EditorScene->GetName());
					CameraEntity = EditorScene->CreateEntity("EditorCamera");
					LK_CORE_INFO("Adding editor camera component to scene");
					LCameraComponent& CameraComp = CameraEntity.AddComponent<LCameraComponent>();
					CameraComp.Camera = EditorCamera;
					CameraComp.ProjectionType = EditorCamera.GetProjectionType();
				#endif
				}
			...
			{
				LEntity CameraEntity = EditorScene->FindEntity("EditorCamera");
				if (CameraEntity)
				{
					LK_CORE_INFO("Found editor camera entity in editor scene '{}'", EditorScene->GetName());
				}
			}
		}
	}
	...
	void LEditorLayer::UI_SetWindowFocus(const ImGuiID WindowID)
	{
		...
		{
			LK_CORE_WARN_TAG("Editor", "Failed to set window focus to {}", WindowID);
		}
	}
	...
	void LEditorLayer::UI_Debug_EditorViewportInfo()
	{
		...
		ImGui::Indent();
		{
			UI::FScopedColor TextColorLastFrame(ImGuiCol_Text, RGBA32::Text::Darker);
		...
		UI::Font::Push("Larger");
		ImGui::Text("Input Cache");
		UI::Font::Pop();
		ImGui::Indent();
		{
			ImGui::Text("Mouse: %s", EditorContext.Input.bMouseEnabled ? "Enabled" : "Disabled");
			ImGui::Text("Keyboard: %s", EditorContext.Input.bKeyboardEnabled ? "Enabled" : "Disabled");
			{
				UI::FScopedColor TextColor(ImGuiCol_Text, RGBA32::Text::Darker);
				ImGui::Text("(Last Toggle) Focused Window Name: %s", (EditorContext.Input.LastToggleFocusedWindowName != "") ? EditorContext.Input.LastToggleFocusedWindowName : "NULL");
				ImGui::Text("(Last Toggle) Focused Window ID: %lld", EditorContext.Input.LastToggleFocusedWindowID);
			}
		}
		ImGui::Unindent();
		...
		if (ImGui::TreeNode("Viewport Bounds, Size and Position"))
		{
