...
namespace LkEngine {
	...
	namespace VKUtils {
		...
		inline VkAttachmentLoadOp GetVkAttachmentLoadOp(const FramebufferSpecification& specification, const FramebufferTextureSpecification& attachmentSpecification)
		{
			...
			{
				if (Image::IsDepthFormat(attachmentSpecification.Format))
		...
		: m_Specification(specification)
	{
		...
		if (!m_Specification.ExistingFramebuffer)
		{
			for (auto& attachmentSpec : m_Specification.Attachments.Attachments)
			{
				...
				{
					if (Image::IsDepthFormat(attachmentSpec.Format))
				...
				else if (Image::IsDepthFormat(attachmentSpec.Format))
				{
		...
		LK_CORE_ASSERT(specification.Attachments.Attachments.size());
	...
	void VulkanFramebuffer::Release()
	{
		if (m_Framebuffer)
		{
			...
			{
				uint32_t attachmentIndex = 0;
				for (Ref<VulkanImage> image : m_AttachmentImages)
				{
	...
	{
		// LK_CORE_TRACE("VulkanFramebuffer::RT_Invalidate ({})", m_Specification.DebugName);
		...
		for (const auto& attachmentSpec : m_Specification.Attachments.Attachments)
		{
			if (Image::IsDepthFormat(attachmentSpec.Format))
			{
				...
				{
					Ref<Image> existingImage = m_Specification.ExistingImages.at(attachmentIndex);
					LK_CORE_ASSERT(Image::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach non-depth image as depth attachment");
					m_DepthAttachmentImage = existingImage;
				}
				else
				{
					Ref<VulkanImage> depthAttachmentImage = m_DepthAttachmentImage.As<VulkanImage>();
				...
				attachmentDescription.format = VKUtils::VulkanImageFormat(attachmentSpec.Format);
				attachmentDescription.samples = VK_SAMPLE_COUNT_1_BIT;
				attachmentDescription.loadOp = VKUtils::GetVkAttachmentLoadOp(m_Specification, attachmentSpec);
			...
			{
				//LK_CORE_ASSERT(!m_Specification.ExistingImage, "Not supported for color attachments");
				...
				{
					Ref<VulkanFramebuffer> existingFramebuffer = m_Specification.ExistingFramebuffer.As<VulkanFramebuffer>();
					Ref<Image> existingImage = existingFramebuffer->GetImage(attachmentIndex);
					colorAttachment = m_AttachmentImages.emplace_back(existingImage).As<VulkanImage>();
				}
				else if (m_Specification.ExistingImages.find(attachmentIndex) != m_Specification.ExistingImages.end())
#endif
				if (m_Specification.ExistingImages.find(attachmentIndex) != m_Specification.ExistingImages.end())
				{
					Ref<Image> existingImage = m_Specification.ExistingImages[attachmentIndex];
					LK_CORE_ASSERT(!Image::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach depth image as color attachment");
					colorAttachment = existingImage.As<VulkanImage>();
				...
				else
				{
					if (createImages)
					{
						...
						spec.Width = (uint32_t)(m_Width * m_Specification.Scale);
						spec.Height = (uint32_t)(m_Height * m_Specification.Scale);
						colorAttachment = m_AttachmentImages.emplace_back(Image::Create(spec)).As<VulkanImage>();
						LK_CORE_VERIFY(false);

					}
					else
					{
						Ref<Image> image = m_AttachmentImages[attachmentIndex];
						ImageSpecification& spec = image->GetSpecification();
						spec.Width = (uint32_t)(m_Width * m_Specification.Scale);
						spec.Height = (uint32_t)(m_Height * m_Specification.Scale);
						colorAttachment = image.As<VulkanImage>();
		...
		{
			Ref<VulkanImage> image = m_AttachmentImages[i].As<VulkanImage>();
			...
			LK_CORE_ASSERT(attachments[i]);
		}

		if (m_DepthAttachmentImage)
		{
			Ref<VulkanImage> image = m_DepthAttachmentImage.As<VulkanImage>();
			if (m_Specification.ExistingImage && image->GetSpecification().Layers > 1)
			{
				LK_CORE_ASSERT(m_Specification.ExistingImageLayers.size() == 1, "Depth attachments do not support deinterleaving");
			...
			LK_CORE_ASSERT(attachments.back());
		}
