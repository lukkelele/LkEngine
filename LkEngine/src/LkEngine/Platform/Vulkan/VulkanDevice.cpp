...
namespace LkEngine {
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			LK_CORE_TRACE_TAG("Renderer", "Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		LK_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		LK_CORE_ASSERT(queueFamilyCount > 0, "");
		...
		if (extCount > 0)
		{
			...
			{
				LK_CORE_TRACE_TAG("Renderer", "Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					//LK_CORE_TRACE_TAG("Renderer", "  {0}", ext.extensionName);
				}
			}
		}
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		LK_CORE_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}
