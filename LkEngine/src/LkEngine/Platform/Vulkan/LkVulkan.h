...
#include "LkEngine/Renderer/PipelineSpecification.h"
...
namespace LkEngine {
	...
	namespace VKUtils {
		...
		inline void VulkanCheckResult(VkResult result, const char* file, int line)
		{
			if (result != VK_SUCCESS)
			{
				//LK_ERROR("VkResult is '{0}' in {1}:{2}", Utils::VKResultToString(result), file, line);
				if (result == VK_ERROR_DEVICE_LOST)
				{
					using namespace std::chrono_literals;
					std::this_thread::sleep_for(3s);
...
{\
	VkResult res = (f);\
	::LkEngine::VKUtils::VulkanCheckResult(res, __FILE__, __LINE__);\
}


namespace LkEngine::VKUtils
{
